...
/*   Updated: 2025/11/04 13:57:27 by aravelom         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	cast_single_ray(t_data *data, int col)
{
	...
	camera_x = 2 * col / (double)data->screen_width - 1;
	...
	if (ret.side == 0)
		...
		ret.perp_dist = (ret.map_y - data->player.pos_y + (1 - ret.step_y) / 2)
			/ ret.ray_dir_y;
	ret.line_height = data->screen_height / ret.perp_dist;
...
void	raycasting(t_data *data)
{
	...
	while (col < data->screen_width)
	{
		if (col < data->mini_map->screen_width)
			draw_rays(data, col);
