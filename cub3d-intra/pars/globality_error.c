...
/*   Updated: 2025/11/27 12:26:41 by marrandr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_data_color(char **s)
{
	...
	size = count_word_split(s);
	i = 0;
	if (size == 5 && ft_ischar(s[1], ',') == 1 && ft_ischar(s[3], ',') == 1)
		return (1);
...
int	check_a_line(char *s, int *check, char **split, t_data **data)
{
	...
	count = ft_count_word(s, " \t\v\f\r");
	split = ft_split_exam(s, " \t\v\f\r");
	if (!split)
	{
		perror("Malloc: ");
		ft_free_data(data);
